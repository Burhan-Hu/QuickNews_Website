...
from core.stopwords import BASE_STOP_WORDS
...
class ContentProcessor:
    ...
    def _tokenize(self, text):
        """
        统一分词（与 xml_api.py 的 _tokenize_query 保持一致）
        中英文共用同一套逻辑：jieba.lcut → 清洗 → 停用词过滤 → 保留长度≥2
        """
        if not text:
            return []
        
        # 清洗：与 xml_api.py 完全一致（保留空格，Unicode范围 \u4e00-\u9fa5）
