...
def main():
    ...
    print("基于TiDB Cloud + NewsAPI/RSS混合抓取")
    print("=" * 60)
    ...
    if not test_connection():
        ...
        print("  3. TiDB Cloud是否允许你的IP访问（Console -> Security -> IP Allowlist）")
        input("按Enter键退出...")
        sys.exit(1)
    ...
    try:
        ...
        if rss_articles:
            ...
            processed = [a for a in processed if a is not None]  # 过滤空数据
            saved = scheduler.storage.save_articles(processed)
            print(f"[Init] 成功保存 {saved} 条新闻到TiDB")
        else:
            print("[Init] 本次未获取到新闻，将在30分钟后重试")

        # 在首次抓取时添加HTML源
        print("\n[Init] 执行国内HTML网站抓取...")
        html_articles = scheduler.fetcher.fetch_html_domestic()
        if html_articles:
            processed = [scheduler.processor.process_article(a) for a in html_articles]
            processed = [a for a in processed if a is not None]  # 过滤空数据
            saved = scheduler.storage.save_articles(processed)
            print(f"[Init] HTML源保存: {saved}条")
