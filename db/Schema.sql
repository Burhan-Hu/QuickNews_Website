...
BEGIN
    ...
    --  插入国家代码索引（XPath: /metadata/country，权重1.5）
    IF v_country IS NOT NULL THEN
        -- 插入国家代码本身（如 US, CN）
        INSERT IGNORE INTO inverted_index
        (term, news_id, tf_weight, xpath_path, field_type, language, position)
        VALUES
            (v_country, p_news_id, 1.5, '/news/metadata/country', 'metadata', p_lang, 0);

        -- 可选：同时插入国家中文名（如"美国"），提升中文检索体验
        -- 需要从countries表查询，但为避免复杂逻辑，此处仅索引代码
...
proc_main: BEGIN
    ...
    IF v_full_text LIKE '%科技%' OR v_full_text LIKE '%tech%' OR v_full_text LIKE '%AI%'
       ...
       OR v_full_text LIKE '%science%'    OR v_full_text LIKE '%科学%' OR v_full_text LIKE '%机器人%'  THEN
...
SELECT COUNT(*) as today_records FROM media WHERE created_at > DATE_SUB(NOW(), INTERVAL 24 HOUR);
SELECT COUNT(*) as us_news_count
FROM news_countries
